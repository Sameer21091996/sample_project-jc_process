...
//                 // Set values for existing records with defaults
//                 $(row.querySelector('.supplier-name')).val(subletData[i].supplier_name || supplierNames.meanings[0]).trigger('change');
//                 $(row.querySelector('.petty-cash')).val(subletData[i].petty_cash_supplier || pettyCashSuppliers.meanings[0]).trigger('change');
//                 $(row.querySelector('.supplier-site')).val(subletData[i].supplier_site_name || supplierSites.meanings[0]).trigger('change');
//                 $(row.querySelector('.activity-desc')).val(subletData[i].activity_description || activitiesData.activities[0]).trigger('change');

//                 // In showSubletModal function, update these lines:
//                 row.querySelector(`#transactionAmount-${i + 1}`).value = subletData[i].existing_transaction_amount || '0.00';
//                 row.querySelector(`#applyVat-${i + 1}`).checked = subletData[i].existing_apply_vat || false;
//                 row.querySelector('input[placeholder="Enter invoice number"]').value = subletData[i].existing_supplier_invoice_num || '';
//                 row.querySelector('input[type="date"]').value = subletData[i].existing_supplier_invoice_date || getCurrentDate();
//                 row.querySelector(`#vatPercentage-${i + 1}`).value = subletData[i].existing_vat_percentage || '5';
//                 row.querySelector(`#vatAmount-${i + 1}`).value = subletData[i].existing_vat_amount || '0.00';
//                 row.querySelector(`#totalAmount-${i + 1}`).value = subletData[i].existing_total_amount || '0.00';
...
async function processLaborEntries() {
    ...
    const laborEntries = Array.from(rows).map((row, index) => ({
        ...
        incident_number: incidentNumber // Add incident number to each entry
    }));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (incidentNumber) {
        ...
        //             fetch(`/api/servicerequest/Estimate-line/${incidentNumber}`),
        //             fetch(`/api/servicerequest/get-component-values/${incidentNumber}`),
        //             fetch(`/api/servicerequest/estimate-total/${incidentNumber}`)
        //         ]);
        ...
        //         if (estimateResult.status === "success") {
        //             // Separate estimates and actuals
        //             const estimates = estimateResult.data.filter(item => item.record_type === 'Estimate');
        //             const actuals = estimateResult.data.filter(item => 
        //                 item.record_type === 'Actual' || item.record_type === 'ROActivity'
        //             );
                    
        //             // Group both types
        //             const groupedEstimates = groupDataByComponentType(estimates);
        //             const groupedActuals = groupDataByComponentType(actuals);
                    
        //             // Combine them, prioritizing actuals over estimates
        //             const groupedData = {};
        //             Object.keys(groupedEstimates).forEach(componentType => {
        //                 groupedData[componentType] = groupedActuals[componentType] || groupedEstimates[componentType];
        //             });
