class InvoiceGenerator {
    ...
    static async fetchTemplate() {
        const response = await fetch('/Service_request/invoice/');
    ...
    // static generateTotalSummary(totals) {
    //     const vatExclTotal = this.formatCurrency(totals.net_total);
    //     const vatTotal = this.formatCurrency(totals.vat_total);
    //     const balanceDue = this.formatCurrency(parseFloat(totals.net_total) + parseFloat(totals.vat_total));
    ...
    //             <tr>
    //                 <td>Balance Due</td>
    //                 <td class="amount-cell">${balanceDue}</td>
    //             </tr>
    //         </table>
    //     `;
    // }
    ...
    //     const vatAmount = originalAmount * 0.05; // Calculate 5% VAT
    ...
    //             <td rowspan="2">${balanceData.credit_memo || ''}</td>
    //             <td rowspan="2"></td>
    //             <td>${this.formatCurrency(originalAmount)}</td>
    //             <td>${this.formatCurrency(balanceData.loc_available_amount)}</td>
    //             <td>${this.formatCurrency(totals.net_total)}</td>
    //         </tr>
    //         <tr>
    //             <td>Tax Advance Invoice</td>
    //             <td>${this.formatCurrency(vatAmount)}</td>
    //             <td>${this.formatCurrency(balanceData.loc_available_amount)}</td>
    //             <td>${this.formatCurrency(totals.vat_total)}</td>
    //         </tr>
    //         <tr class="adjustment-row">
    //             <td colspan="7">Adjustment for Advance</td>
    //             <td class="amount-cell">${this.formatCurrency(balanceData.loc_available_amount)}</td>
    //             <td class="amount-cell">${this.formatCurrency(parseFloat(totals.net_total) + parseFloat(totals.vat_total))}</td>
    //         </tr>
    //     `;
    // }
    ...
    static downloadInvoice(html, serviceRequestId) {
        const blob = new Blob([html], { type: 'text/html' });
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `invoice_${serviceRequestId}.html`;
...
async function generateAndDownloadInvoice(serviceRequestId) {
    try {
        console.log('Generating invoice for service request:', serviceRequestId);
        const invoiceData = await InvoiceGenerator.fetchInvoiceData(serviceRequestId);
        let templateHtml = await InvoiceGenerator.fetchTemplate();

        if (invoiceData.status === 'success') {
            templateHtml = InvoiceGenerator.populateInvoiceDetails(templateHtml, invoiceData.data.invoice_details);
            const lineItemsHtml = InvoiceGenerator.generateLineItems(invoiceData.data.line_items);
            const totalRowHtml = InvoiceGenerator.generateTotalRow(invoiceData.data.totals);
            const customerBalanceHtml = InvoiceGenerator.generateCustomerBalanceRows(invoiceData.data.customer_balance, invoiceData.data.totals);
            ...
            // Replace the total summary table
            templateHtml = templateHtml.replace(
                /<table style="width: auto; margin-left: auto;">[\s\S]*?<\/table>/,
                totalSummaryHtml
            );

            InvoiceGenerator.downloadInvoice(templateHtml, serviceRequestId);
