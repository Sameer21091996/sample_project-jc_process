...
from Service_request.models import CONSUMABLE, CUSTOMERBALANCE, LABOR, ComponentSubtotal, ServiceRequestHeader, ServiceRequestLine, Sublet
    ...
    try:
        ...
        with transaction.atomic():
            for sublet_data in sublets:
                ...
                service_request_line.estimate_status = 'Processed'
...
def process_labor(request, data: LaborBulkProcessSchema):
    try:
        data = json.loads(request.body)
        ...
        if not labor_entries:
            return JsonResponse({
                "status": "error", 
                "message": "No labor entries provided"
            }, status=400)

        service_request = ServiceRequestHeader.objects.get(incident_number=incident_number)
        
        for item in labor_entries:
            try:
                service_request_line = ServiceRequestLine.objects.get(
                    component_code=item.get('operation_code'),
                    component_type='Labour',
                    service_request=service_request
                )
                ...
                labor_data = {
                    ...
                    'trx_qty': item.get('trx_qty'),
                    'plan_start_date': item.get('plan_start_date'),
                    'plan_end_date': item.get('plan_end_date'),
                ...
                labor = LABOR.objects.filter(id=item.get('id')).first() if item.get('id') else None
                ...
                # service_request_line.actual_quantity = item.get('trx_qty', 0)
                service_request_line.save()

            except ValidationError as ve:
                logger.error(f"Validation error processing labor entry: {ve}")
                return JsonResponse({
                    "status": "error",
                    "message": str(ve)
                }, status=400)
                ...
                logger.error(f"Error processing labor entry: {e}")
                return JsonResponse({
                    "status": "error",
                    "message": f"Error processing labor entry: {str(e)}"
                }, status=500)
        ...
        return JsonResponse({
            "status": "error",
            "message": f"Service request {incident_number} not found"
        }, status=404)
        
    except Exception as e:
        logger.error(f"Unexpected error in process_labor: {e}")
        return JsonResponse({
            "status": "error",
...
@servicerequest_api.post("/actual-line")
def create_actual_line(request, data: ActualLineSchema):
    try:
        incident_number = data.service_request_id
        if not isinstance(incident_number, str):
            incident_number = f"INC-{hex(incident_number)[2:].upper()}"
        ...
        service_request = ServiceRequestHeader.objects.get(incident_number=incident_number)
        net_price = Decimal(str(data.actual_quantity)) * data.selling_price

        actual_line = ServiceRequestLine.objects.create(
            service_request=service_request,
            record_type='Actual',
            component_type=data.component_type,
            activity_line=data.activity_line,
            ...
            net_price=net_price,
            parts_issue_status=data.parts_issue_status,
            ...
            estimated_variation=data.estimated_variation  # This matches the schema field name
        )

        return {
            "status": "success",
            "actual_line_id": actual_line.id,
            "message": "Actual line created successfully"
        }
...
class ActualLineUpdateSchema(Schema):
    adjustment: Optional[Decimal] = None
    discount_percentage: Optional[Decimal] = None
    net_amount: Optional[Decimal] = None
    vat: Optional[Decimal] = None
    ...
    parts_issue_status: Optional[str] = None
    estimated_variation: Optional[Decimal] = None
@servicerequest_api.put("/actual-line/{incident_number}/{line_id}")
def update_actual_line(request, incident_number: str, line_id: int, data: ActualLineUpdateSchema):
    try:
        service_request = ServiceRequestHeader.objects.get(incident_number=incident_number)
        
        actual_line = ServiceRequestLine.objects.get(
            id=line_id,
            service_request=service_request,
            record_type='Actual'
        )
        ...
        if data.adjustment is not None:
            actual_line.adjustment = data.adjustment
        
        if data.discount_percentage is not None:
            actual_line.discount_percentage = data.discount_percentage
            
        if data.net_amount is not None:
            actual_line.net_amount = data.net_amount
            
        if data.vat is not None:
            actual_line.vat = data.vat
            
        if data.final_amount is not None:
            actual_line.final_amount = data.final_amount
            
        if data.parts_issue_status is not None:
            actual_line.parts_issue_status = data.parts_issue_status

        if data.estimated_variation is not None:
            actual_line.estimated_variation = data.estimated_variation
        ...
        return {
            "status": "success",
            "actual_line_id": actual_line.id,
            "message": "Actual line updated successfully"
        }

    except ServiceRequestHeader.DoesNotExist:
        return {"error": f"Service request with incident number {incident_number} not found"}
    except ServiceRequestLine.DoesNotExist:
        return {"error": f"Actual line with ID {line_id} not found"}
    except Exception as e:
        return {"error": str(e)}


# for saving the consumables calculations row data 
@servicerequest_api.post("/save-calculations/")
def save_calculations(request):
    try:
        data = json.loads(request.body)
        incident_number = data.get('incident_number')
        ...
        for calc in calculations:
            line_id = calc.get('line_id')
            service_line = ServiceRequestLine.objects.get(
                id=line_id,
                service_request=service_request,
            ...
            service_line.net_price = Decimal(str(calc.get('net_price', 0)))
            service_line.adjustment = Decimal(str(calc.get('adjustment', 0)))
            service_line.discount_percentage = Decimal(str(calc.get('discount_percentage', 0)))
            service_line.net_amount = Decimal(str(calc.get('net_amount', 0)))
            service_line.vat = Decimal(str(calc.get('vat', 0)))
            service_line.final_amount = Decimal(str(calc.get('final_amount', 0)))
            service_line.estimated_total = Decimal(str(calc.get('estimated_total', 0)))
            service_line.estimated_variation = Decimal(str(calc.get('estimated_variation', 0)))
        ...
        return JsonResponse({
            ...
            "incident_number": incident_number,
            "component_type": component_type,
            "updated_count": len(calculations)
        })
        ...
        return JsonResponse({
            "status": "error", 
            "message": "One or more service request lines not found"
        })
    except Exception as e:
        return JsonResponse({
            "status": "error",
...
# For saving the sublet calculations row data
@servicerequest_api.post("/save-sublet-calculations/")
def save_sublet_calculations(request):
    try:
        data = json.loads(request.body)
        incident_number = data.get('incident_number')
        calculations = data.get('calculations', [])
        ...
        return JsonResponse({
            "status": "success",
            "message": "Sublet calculations saved successfully",
            "incident_number": incident_number,
            "updated_count": len(calculations)
        })
...
# For saving the labour calculations row data
@servicerequest_api.post("/save-labour-calculations/")
def save_labour_calculations(request):
    try:
        data = json.loads(request.body)
        incident_number = data.get('incident_number')
        calculations = data.get('calculations', [])
        ...
        return JsonResponse({
            "status": "success",
            "message": "Labour calculations saved successfully",
            "incident_number": incident_number,
            "updated_count": len(calculations)
        })
...
# For Saving the Actual Data
@servicerequest_api.post("/save-actual-calculations/")
def save_actual_calculations(request):
    try:
        data = json.loads(request.body)
        incident_number = data.get('incident_number')
        component_type = data.get('component_type')
        ...
        print(f"Processing data for incident {incident_number}:")
        ...
        for calc in calculations:
            ...
            existing_record = ServiceRequestLine.objects.filter(
                service_request=service_request,
                record_type='Actual',
                component_type=component_type,
                component_code=calc.get('component_code'),
                activity_line=calc.get('activity_line')
            ).first()

            if existing_record:
                # Update existing record
                existing_record.adjustment = Decimal(str(calc.get('adjustment', 0)))
                existing_record.discount_percentage = Decimal(str(calc.get('discount_percentage', 0)))
                existing_record.net_amount = Decimal(str(calc.get('net_amount', 0)))
                existing_record.vat = Decimal(str(calc.get('vat', 0)))
                existing_record.final_amount = Decimal(str(calc.get('final_amount', 0)))
                existing_record.estimated_variation = Decimal(str(calc.get('estimated_variation', 0)))
                existing_record.save()
                processed_records.append({'id': existing_record.id, 'action': 'updated'})
            else:
                # Create new record
                new_record = ServiceRequestLine.objects.create(
                    service_request=service_request,
                    record_type='Actual',
                    component_type=component_type,
                    component_code=calc.get('component_code'),
                    component_description=calc.get('component_description'),
                    activity_line=calc.get('activity_line'),
                    activity_desc=calc.get('activity_desc'),
                    actual_quantity=calc.get('actual_quantity'),
                    selling_price=Decimal(str(calc.get('selling_price', 0))),
                    net_price=Decimal(str(calc.get('net_price', 0))),
                    adjustment=Decimal(str(calc.get('adjustment', 0))),
                    discount_percentage=Decimal(str(calc.get('discount_percentage', 0))),
                    net_amount=Decimal(str(calc.get('net_amount', 0))),
                    vat=Decimal(str(calc.get('vat', 0))),
                    final_amount=Decimal(str(calc.get('final_amount', 0))),
                    estimated_total=Decimal(str(calc.get('estimated_total', 0))),
                    estimated_variation=Decimal(str(calc.get('estimated_variation', 0)))
                )
                processed_records.append({'id': new_record.id, 'action': 'created'})

        print(f"Successfully processed {len(processed_records)} records")
        print("Details:", processed_records)

        return JsonResponse({
            "status": "success",
            "message": "Actual calculations processed successfully",
            "incident_number": incident_number,
            "processed_records": processed_records
        })

    except Exception as e:
        print(f"Error processing actual calculations: {str(e)}")
        return JsonResponse({
            "status": "error",
            "message": str(e)
        })
...
@servicerequest_api.post("/save-component-subtotal/")
def save_component_subtotal(request):
    try:
        data = json.loads(request.body)
        incident_number = data.get('incident_number')
        component_type = data.get('component_type')
        ...
        subtotal, created = ComponentSubtotal.objects.update_or_create(
            service_request=service_request,
            component_type=component_type,
            defaults={
                'net_price': Decimal(str(subtotal_data.get('net_price', 0))),
                'adjustment': Decimal(str(subtotal_data.get('adjustment', 0))),
                'discount': Decimal(str(subtotal_data.get('discount', 0))),
                'net_amount': Decimal(str(subtotal_data.get('net_amount', 0))),
                'vat': Decimal(str(subtotal_data.get('vat', 0))),
                'final_amount': Decimal(str(subtotal_data.get('final_amount', 0))),
                'estimated_total': Decimal(str(subtotal_data.get('estimated_total', 0))),
                'estimated_variation': Decimal(str(subtotal_data.get('estimated_variation', 0)))
            }
        )

        return JsonResponse({
            "status": "success",
            "message": f"{component_type} subtotal {'created' if created else 'updated'} successfully",
            "incident_number": incident_number,
            "component_type": component_type
        })
...
@servicerequest_api.get("/component-subtotals/{incident_number}")
def get_component_subtotals(request, incident_number):
    try:
        service_request = ServiceRequestHeader.objects.get(incident_number=incident_number)
        subtotals = ComponentSubtotal.objects.filter(service_request=service_request)
        
        data = []
        for subtotal in subtotals:
            data.append({
                'component_type': subtotal.component_type,
                'net_price': str(subtotal.net_price),
                'adjustment': str(subtotal.adjustment),
                'discount': str(subtotal.discount),
                'net_amount': str(subtotal.net_amount),
                'vat': str(subtotal.vat),
                'final_amount': str(subtotal.final_amount),
                'estimated_total': str(subtotal.estimated_total),
                'estimated_variation': str(subtotal.estimated_variation)
            })
